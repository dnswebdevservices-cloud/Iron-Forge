...
(function () {
  ...
  function setMenu(open) {
    ...
    if (use) use.setAttribute("href", open ? "#i-x" : "#i-list");
  }
  ...
  if (window.gsap && window.ScrollTrigger) {
    ...
    // Horizontal pan, canonical form: pin at "top top", scroll length equal
    // to the horizontal travel, scrubbed. Desktop only.
    mm.add("(min-width: 1024px) and (prefers-reduced-motion: no-preference)", function () {
      var section = document.querySelector(".equip");
      var pin = section && section.querySelector(".equip__pin");
      if (!section || !pin || !rail) return;

      section.classList.add("is-pinned");

      function distance() {
        return Math.max(0, rail.scrollWidth - document.documentElement.clientWidth);
      }
